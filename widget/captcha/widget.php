...
/*
	...
	$Id: captcha.class.php 753 2008-11-14 06:48:25Z cnteacher $
*/
class widget_captcha {
	var $code;			//100000-999999 范围内随机
	var $type 	= 0;		//0 英文图片验证码  1 中文图片验证码(x)  2 Flash 验证码  3 语音验证码
	var $width 	= 90;		//宽度
	var $height = 35;		//高度
	var $background	= 0;		//随机图片背景
	...
	function captchaconvert(&$captcha) {
		$s = sprintf('%04s', base_convert($captcha, 10, 20));
		$captchaunits = 'CEFHKLMNOPQRSTUVWXYZ';
		$captcha = '';
		for($i = 0; $i < 4; $i++) {
			$unit = ord($s{$i});
	...
	function __construct(){
		if (empty($this->fontpath)) $this->fontpath = dirname(__FILE__).'/font/';
		if (empty($this->datapath)) $this->datapath = dirname(__FILE__).'/font/';
		if (empty($this->includepath)) $this->includepath = dirname(__FILE__).'/';
		if ($this->type==1) $this->type = 0;
		$this->code = mt_rand(100000,999999);
		$this->type==2 && !extension_loaded('ming') && $this->type = 0;
		$this->width = $this->width >= 0 && $this->width <= 200 ? $this->width : 180;
		$this->height = $this->height >= 0 && $this->height <= 80 ? $this->height : 70;
		$this->captchaconvert($this->code);
		if (class_exists('cookie')) cookie::set('captcha', $this->code);
	...
	function image() {
		...
		if($this->animator == 1 && function_exists('imagegif')) {
			include_once $this->includepath.'gifmerge.php';
	...
	function ttffont() {
		$captcha = $this->code;
		$charset = function_exists('gc') ? gc('base.charset') : 'utf-8';
		$captcharoot = $this->type ? $this->fontpath.'ch/' : $this->fontpath.'en/';
		$dirs = opendir($captcharoot);
		$captchattf = array();
		...
		$captchalength = 4;
		if($this->type && !empty($captchattf)) {
			if(strtoupper($charset) != 'UTF-8') {
				include $this->includepath.'chinese.class.php';
				$cvt = new Chinese($charset, 'utf8');
				$captcha = $cvt->Convert($captcha);
			}
			$captcha = array(substr($captcha, 0, 3), substr($captcha, 3, 3));
			$captchalength = 2;
		}
		$widthtotal = 0;
		for($i = 0; $i < $captchalength; $i++) {
			$font[$i]['font'] = $captcharoot.$captchattf[array_rand($captchattf)];
			...
			$box = imagettfbbox($font[$i]['size'], 0, $font[$i]['font'], $captcha[$i]);
			$font[$i]['zheight'] = max($box[1], $box[3]) - min($box[5], $box[7]);
			$box = imagettfbbox($font[$i]['size'], $font[$i]['angle'], $font[$i]['font'], $captcha[$i]);
			...
			$font[$i]['width'] = $font[$i]['width'] > $this->width / $captchalength ? $this->width / $captchalength : $font[$i]['width'];
		...
		for($i = 0; $i < $captchalength; $i++) {
			...
			$this->shadow && imagettftext($this->im, $font[$i]['size'], $font[$i]['angle'], $x + 1, $y + 1, $text_shadowcolor, $font[$i]['font'], $captcha[$i]);
			imagettftext($this->im, $font[$i]['size'], $font[$i]['angle'], $x, $y, $text_color, $font[$i]['font'], $captcha[$i]);
			$x += $font[$i]['width'];
		}
	}
	...
	function giffont() {
		...
		if(function_exists('imagecreatefromgif')) {
			$captcharoot = $this->datapath.'gif/';
			$dirs = opendir($captcharoot);
			while($dir = readdir($dirs)) {
				if($dir != '.' && $dir != '..' && file_exists($captcharoot.$dir.'/9.gif')) {
					$captchadir[] = $dir;
				}
			}
		}
		$widthtotal = 0;
		for($i = 0; $i <= 3; $i++) {
			$this->imcodefile = $captchadir ? $captcharoot.$captchadir[array_rand($captchadir)].'/'.strtolower($captcha[$i]).'.gif' : '';
		...
		for($i = 0; $i <= 3; $i++) {
			...
			if($font[$i]['file']) {
				...
				if($this->shadow) {
					$text_shadowcolor = imagecolorallocate($this->im, 255 - $this->fontcolor[0], 255 - $this->fontcolor[1], 255 - $this->fontcolor[2]);
					imagechar($this->im, 5, $x + 1, $y + 1, $captcha[$i], $text_shadowcolor);
				}
				$text_color = imagecolorallocate($this->im, $this->fontcolor[0], $this->fontcolor[1], $this->fontcolor[2]);
				imagechar($this->im, 5, $x, $y, $captcha[$i], $text_color);
			}
			$x += $font[$i]['width'];
		}
	}
