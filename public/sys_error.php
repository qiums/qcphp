...
<?php if(gc('base.run_mode')==='debug'): foreach($traces as $trace): if(is_array($trace)&&!empty($trace["file"])):
...
function getsource($file, $line){
	...
	$start = $line - 3;if ($start < 1) {$start = 1;}
	$end = $line + 3;if ($end > $count) {$end = $count + 1;}
	$returns = array();
	for ($i = $start; $i <= $end; $i++) {
		if( $i == $line ){
